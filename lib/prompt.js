...
Every fact appears exactly once, in exactly one group. If a rule is already carried inside a task in THE WORK or THE LEGWORK, it does not also appear in THE RULES. Before you answer, read all four lists together and delete anything you have said twice. When two items refer to the same named resource or reproduce the same list, give it in full once - in THE RULES if it belongs there - and refer to it briefly everywhere else. A long title or option list repeated in full makes two different actions look like one repeated one.
...
A task carries the specifics it needs to be actionable - but there is a ceiling. If those specifics are a list of three or more things, or would push the item past twenty-five words, put the list in THE RULES and keep the task short. A bullet that takes two breaths to read is not a checklist item. Exclusions, banned sources, and long option lists almost always belong in THE RULES for this reason.

If the source offers alternatives the student chooses between - two or more prompts, topics or questions where only one is answered - then choosing is the first item here, each alternative goes in THE RULES as its own separate item, never bundled into one, and the remaining items describe what the student does whatever they picked. Never write one item per alternative. The moment they choose, every item about the others is noise.

This splitting applies ONLY to alternatives the student picks between. Any other list - banned sources, suggested talking points, criteria to choose among, examples - stays as a single item with the options separated by semicolons. Consolidating a list never means dropping any of its contents, and never licenses dropping anything else from THE RULES. A shorter RULES that has lost a deadline, a weight or a penalty is a failure, not an improvement.
...
Do not invent preparatory steps. A lookup belongs in THE LEGWORK only when the source itself points at something it does not provide, or leaves one of its own stated requirements unresolvable. Never add a lookup the source never raises, however useful it might seem - if the source says nothing about a late policy, neither do you. And if the source already told them something, they do not need an item telling them to go confirm it.
